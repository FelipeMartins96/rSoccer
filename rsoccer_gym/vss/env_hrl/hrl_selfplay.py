...
class VSSHRLSelfEnv(gym.Env):
    ...
    def __init__(
        ...
        self.m_reward_weights = np.array([m_w_goal, m_w_ball_grad])
        self.w_reward_weights = np.array([w_w_dist])
    ...
    def get_spaces_m(self):
        ...
        m_observation_space = gym.spaces.Box(
            ...
            shape=(4 + ((self.n_robots_blue + self.n_robots_yellow) * 7),),
            dtype=np.float32,
        )
    ...
    def _get_obs_m(self):
        ...
        yellow = np.concatenate(
            [
                -self.observations.ball,
                (self.mirror_entity * self.observations.yellow).flatten(),
                (self.mirror_entity * self.observations.blue).flatten(),
            ]
        )

        return np.stack([blue, yellow])
