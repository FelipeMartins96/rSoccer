...
class VSSJALEnv(VSSBaseEnv):
    def __init__(self, n_robots_control=3):
        ...
        self.ou_actions = [(OrnsteinUhlenbeckAction(self.action_space, dt=self.time_step))]
    ...
    def _get_commands(self, actions):
        ...
        actions = self.ou_actions[0].sample().reshape(-1,2)
        for i in range(self.n_robots_yellow):
            v_wheel0, v_wheel1 = self._actions_to_v_wheels(actions[i])
        ...
        if self.frame.ball.x > (self.field.length / 2):
            ...
            if self.last_frame is not None:
                ...
                for idx in range(self.n_robots_control):
                    # Calculate Move ball
                    move_rewards.append(self._move_reward(robot_idx=idx))
                    # Calculate Energy penalty
                    energy_penalties.append(self._energy_penalty(robot_idx=idx))

                move_reward = np.mean(move_rewards)
                energy_penalty = np.mean(energy_penalties)

                reward = grad_ball_potential * w_ball_grad + \
                    move_reward * w_move + \
                    energy_penalty * w_energy  # noqa 
